...
考虑到各位小伙伴没有学习过Vue等前端框架，这里我们依然使用前端模版进行魔改。只不过现在我们的前端页面需要单独进行部署，而不是和后端揉在一起，这里我们需要先创建一个前端项目，依赖只需勾选SpringWeb即可，**主要用作反向代理前端页面（可以用Nginx效果更好）**：
...
public class RestBean<T> {
	...
	// 使用FastJson2，将当前对象转换为JSON格式的字符串（允许空值）用于返回
...
> 当你使用 Postman 发送请求时，通常不会遇到 CORS（跨源资源共享）问题，这是因为 Postman 发送的请求是从你的本地计算机的环境中发出的，而不是从浏览器中发出的。CORS 是一种浏览器安全策略，用于保护 Web 应用程序免受跨域请求的潜在风险。
>
> 浏览器实施了同源策略，该策略要求在默认情况下，从一个源加载的文档或脚本无法与来自不同源的资源进行交互。源指的是协议、域名和端口号的组合。如果浏览器检测到请求违反了同源策略，它将阻止该请求，并在控制台中报告 CORS 错误。
...
public class UserController {
    ...
    public RestBean<String> username() {
        return RestBean.success(SecurityContextHolder.getContext().getAuthentication().getName());
    }
}
```
...
不过我们发现，我们的一些响应还是不完善，比如用户没有登录，**默认还是会302重定向（在没有记住cookie的情况下）**，但是实际上我们只需要告诉前端没有登录就行了，所以说我们修改一下未登录状态下返回的结果：
...
> 在 Spring Security 的 `exceptionHandling` 配置中，你可以定义和处理不同类型的异常情况。以下是一些常见的用法：
>
> 1. `.accessDeniedPage("/access-denied")`: 使用自定义的错误页面来处理访问被拒绝的情况。当用户试图访问其权限不足的资源时，将被重定向到指定的错误页面。
> 2. `.authenticationEntryPoint(authenticationEntryPoint)`: 自定义身份验证入口点，用于处理未经身份验证的用户尝试访问受保护资源的情况。可以提供一个实现了 `AuthenticationEntryPoint` 接口的自定义类来处理身份验证失败的情况。
> 3. `.defaultAuthenticationEntryPointFor(entryPoint, requestMatcher)`: 为指定的请求匹配器配置默认的身份验证入口点。可用于根据请求的特定条件选择不同的身份验证入口点。
> 4. `.defaultAccessDeniedHandlerFor(accessDeniedHandler, requestMatcher)`: 为指定的请求匹配器配置默认的访问被拒绝处理器。可用于根据请求的特定条件选择不同的访问被拒绝处理器。
> 5. `.authenticationHandler(authenticationHandler)`: 自定义身份验证处理器，用于处理身份验证失败的情况。可以提供一个实现了 `AuthenticationFailureHandler` 接口的自定义类来定义身份验证失败的处理逻辑。
> 6. `.accessDeniedHandler(accessDeniedHandler)`: 自定义访问被拒绝处理器，用于处理访问被拒绝的情况。可以提供一个实现了 `AccessDeniedHandler` 接口的自定义类来定义访问被拒绝的处理逻辑。

现在有三个方法，但是实际上功能都是一样的，我们可以把它们**整合为同一个方法**：
...
* **加密算法：**加密算法分为对称加密和非对称加密，非对称加密的安全性会更高一些，包括HTTPS的隐私信息正是使用非对称加密来保障传输数据的安全（当然HTTPS并不是单纯地使用非对称加密完成的，感兴趣的可以去了解一下）
...
SpringSecurity中并没有为我们提供预设的JWT校验模块（只有OAuth2模块才有，但是知识太超前了，我们暂时不讲解）这里我们只能手动进行整合

> Spring Security 和 OAuth2 是两个不同但相关的技术。Spring Security 是一个用于应用程序安全的框架，它提供了许多功能来帮助保护应用程序中的资源和服务，如身份验证、授权、防止 CSRF 攻击等。而 OAuth2 是一个用于授权的协议，它定义了一种流程，允许用户授权第三方应用程序访问他们在另一个服务提供商上的资源。如果只是拿来用于颁布token的话，二者没区别，只是类名不一样。

JWT可以存放在Cookie或是请求头中，不过不管哪种方式，我们都可以通过Request获取到对应的JWT令牌，这里我们使用比较常见的请求头携带JWT的方案，客户端发起的请求中会携带这样的的特殊请求头：
...
> Bearer更安全，因为：
>
> - 令牌可以是随机生成的、有限期的，并且可以进行加密，提供了更好的安全性。
> - 令牌通常不包含敏感信息，因此即使被窃取，也难以获得用户的实际凭据。
...
接着我们需要自行实现一个 JwtAuthenticationFilter 加入到 SpringSecurity 默认提供的过滤器链（有关 SpringSecurity 的实现原理介绍，我们在SSM中已经详细讲解过，各位小伙伴可以回顾一下）中，用于处理请求头中携带的JWT令牌，并配置登录状态：
    ...
    protected void doFilterInternal(HttpServletRequest request, 
        ...
        if (authorization != null && authorization.startsWith("Bearer ")) {
            ...
            if(user != null) {
                ...
                // 用于设置Authentication对象的详细信息（details）
                // 可以在用户登录成功后，通过authentication.getDetails()方法获取用户的IP地址，或者在日志中记录用户的会话ID等信息
...
public class SecurityConfiguration {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        return http
          		// 其他跟之前一样，就省略掉了
                ...  
                // 将Session管理创建策略改成无状态，这样SpringSecurity就不会创建会话了，也不会采用之前那套机制记录用户，
                // 因为现在我们可以直接从JWT中获取信息
...
<script>
    ...
    function getInfo() {
        axios.post('http://localhost:8081/api/auth/login', {
            ...
            if(data.code === 200) {
                //将得到的JWT令牌存到本地的sessionStorage用于本次会话
...
这里我们以黑名单机制为例，让用户退出登录之后，无法再次使用之前的JWT进行操作，首先我们需要**给JWT额外添加一个用于判断的唯一标识符**，这里就用UUID，确保这样不可能产生两个完全一样的 token：
...
看狂神的spring boot 了解分布式微服务基础

## 项目实战

[前后端分离项目 - 基于Session的校验方案]: https://www.bilibili.com/video/BV1rT411W7QM/?share_source=copy_web&amp;vd_source=d3666c2043c363d538f217512a5c5be0
[前后端分离项目 - 基于JWT的校验方案]: https://www.bilibili.com/video/BV1Pz4y1W7TN/?share_source=copy_web&amp;vd_source=d3666c2043c363d538f217512a5c5be0
