...
那么现在我们需要调用其他微服务提供的接口，该怎么做呢？我们直接在微服务中创建一个需要的对应服务的接口类即可：
...
那么现在，由于用户服务和图书服务不可用，所以查询借阅信息的请求肯定是没办法正常响应的，这时我们可以提供一个备选方案

也就是说当服务出现异常时，返回我们的备选方案：
...
除了对服务的降级和熔断处理，我们也可以对其进行实时监控，只需要安装监控页面即可，这里我们创建一个新的项目：
...
> 注册中心的主要作用包括：
>
> - 服务注册：微服务在启动时将自己的实例信息注册到注册中心，包括服务ID、主机名、IP地址、端口等。
> - 服务发现：其他微服务可以通过查询注册中心获取可用的服务实例列表，从而实现服务之间的通信和调用。
>
> 网关的主要作用包括：
>
> - 路由转发：根据请求的URL路径将请求转发到相应的微服务实例。
> - 协议转换：将不同的协议（如HTTP、WebSocket等）转换为内部微服务所需的协议。
> - 安全认证：对请求进行身份验证和授权，确保只有合法的用户可以访问受保护的资源。
> - 负载均衡：将请求平衡地分发到多个微服务实例上，以提高系统的可用性和性能。
...
> WebFlux是Spring Framework 5引入的一种反应式编程模型，用于构建响应式、非阻塞的Web应用程序。它提供了一种基于异步和事件驱动的编程模式，以处理高并发的请求和响应。
>
> WebFlux的主要目标是提供一种响应式的编程模型，以便应对传统的阻塞式I/O模型所面临的性能和可伸缩性限制。它使用了Reactor库作为其反应式流处理的基础，并提供了支持响应式的Web框架和服务器。
...
```java
@Component   //需要注册为Bean，需要实现 GlobalFilter
