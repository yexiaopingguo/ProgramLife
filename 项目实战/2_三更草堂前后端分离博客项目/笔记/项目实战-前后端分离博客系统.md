...
[B站最通俗易懂手把手SpringBoot+Vue项目实战-前后端分离博客项目-Java项目]: https://www.bilibili.com/video/BV1hq4y1F7zk/?share_source=copy_web&amp;vd_source=d3666c2043c363d538f217512a5c5be0
...
博客项目包含前台和后台两套系统，实际上两套后端系统k的很多内容是可能重复的。这里如果我们只是单纯的创建两个后端工程。那么就会有大量的重复代码，并且需要修改的时候也需要修改两次，这就是代码复用性不高。
...
| ------ | ---------------------------------- |
| GET    | 用于信息获取，安全且幂等           |
| POST   | 一般用于创建数据，不是安全和幂等的 |
| PUT    | 用于创建或完整更新数据，安全且幂等 |
| DELETE | 用于删除数据，安全且幂等           |
...
mybatis-plus:
  ...
  global-config:
    db-config:
      logic-delete-field: delFlag    # 指定用于逻辑删除的字段名
      logic-delete-value: 1    # 指定逻辑删除字段的删除值。
      logic-not-delete-value: 0    # 指定逻辑删除字段的非删除值。
      id-type: auto    # 根据数据库类型自动选择生成策略
~~~~
...
​	这些文件应该写在`sangeng-framework`模块下面，让前台和后台模块去直接通过依赖父类获得

​	可以通过 IDEA 的插件`EasyCode`自动生成代码：
...
⑤ 在`sangeng-blog`模块中创建Controller测试接口
...
> 注意可能会出现的问题：
>
> 1. 依赖问题：因为是从其他模块导入重复的类，依赖可能没有更新，所以 maven 要重新 clean 和 install 父类依赖
> 2. 要记得在启动类下面配置 mapper 模块类中的@MapperScan("xxx.xxx.xxx")
> 3. 要配置数据库表映射 @TableName("xxx")
...
​	需要查询浏览量最高的前10篇文章的信息（按照浏览量进行降序排序），展示出文章标题和浏览量，并且能让用户自己跳转到具体感兴趣的文章进行详情浏览
...
@JsonInclude(JsonInclude.Include.NON_NULL)    // 如果一个属性的值为 null，则在生成的 JSON 字符串中将不包含该属性
...
public class ArticleServiceImpl extends ServiceImpl<ArticleMapper, Article> implements ArticleService {
    ...
    public ResponseResult hotArticleList() {
        ...
        //MyBatis-Plus 的分页查询功能，设置最多只查询10条
...
MyBatis-Plus 的分页查询功能需要配置PaginationInnerInterceptor分页插件

> [mybatis-plus分页查询]: https://developer.aliyun.com/article/1055589#slide-1
...
public class ArticleServiceImpl extends ServiceImpl<ArticleMapper, Article> implements ArticleService {
    ...
    public ResponseResult hotArticleList() {
        //查询热门文章 封装成ResponseResult返回
        LambdaQueryWrapper<Article> queryWrapper = new LambdaQueryWrapper<>();
        //必须是正式文章（这里不允许直接在代码中使用字面值，都需要定义成常量来使用）
...
public class BeanCopyUtils {
    ...
    public static <O,V> List<V> copyBeanList(List<O> list,Class<V> clazz){
        //使用了 Java 8 中的流式操作（Stream API）来对一个列表进行处理和转换
        return list.stream()
...
> [mybatis-plus分页查询详解]: https://developer.aliyun.com/article/1055589#slide-1
...
> 如果数据库中的数据有很多(假设有十几万条)的时候，将数据全部展示出来肯定不现实，这时可以使用分页来解决这个问题，每次只展示一页的数据，比如：一页展示10条数据，如果还想看其他的数据，可以通过点击页码进行查询。
...
//        }
    ...
    public ResponseResult articleList(Integer pageNum, Integer pageSize, Long categoryId) {
        ...
        // categoryId可以不存在，如果有categoryId 就要查询时要和传入的相同
...
~~~~java
@TableField(exist = false)    // 设置可以不存在这个栏位
private String categoryName;
~~~~

#### 3.3.4 FastJson配置时间⭐

在 WebConfig 中配置 json 库，这里选择 FastJson（性能好）
...
##### 在`sangeng-blog`模块中创建SecurityConfig
...
public class BlogLoginServiceImpl implements BlogLoginService {
    ...
    public ResponseResult login(User user) {
        ...
        //该方法将尝试使用提供的用户名和密码进行身份验证。
        //如果验证成功，将返回一个经过身份验证的 Authentication 对象表示身份验证成功的结果
...
~~~~java
@Service
public class UserDetailsServiceImpl implements UserDetailsService {    // UserDetailsService 应该再继承 AuthorizeService
...
> 在 Java 中，"claims"（声明）通常指的是 JSON Web Token（JWT）中的声明部分。JWT 是一种用于在不同的系统之间安全传递信息的标准方法，它由三个部分组成：头部（header）、载荷（payload）和签名（signature）。
>
> 在 JWT 的载荷部分，可以包含一些自定义的声明（claims），用于携带关于用户或其他相关信息的数据。这些声明是以键值对的形式存在的，可以根据实际需求自定义添加。常见的 JWT 声明包括以下几种：
>
> 1. "iss"（Issuer）：表示发行人，即 JWT 的签发者。
> 2. "sub"（Subject）：表示主题，即 JWT 所面向的用户。
> 3. "aud"（Audience）：表示受众，即 JWT 的预期接收者。
> 4. "exp"（Expiration Time）：表示过期时间，即 JWT 的有效期。
> 5. "iat"（Issued At）：表示签发时间，即 JWT 的生成时间。
> 6. "nbf"（Not Before）：表示生效时间，即 JWT 的生效时间，在此之前不可用。
...
![](./img/7a5786bdabccd613af11a875a94442b.png)
...
<img src="./img/c2123582cb6606f8a2b20b7fe371de0.png" style="zoom: 80%;" />
...
[上传图片到七牛云并返回图片URL]: https://blog.csdn.net/j1231230/article/details/80061834
...
[Spring Security - 角色和权限]: https://springdoc.cn/role-and-privilege-for-spring-security-registration/
