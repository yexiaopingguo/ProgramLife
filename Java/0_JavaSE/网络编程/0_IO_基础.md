...
>
> NIO引入了一种非阻塞的I/O模型，它允许单个线程管理多个并发的I/O操作。在NIO中，线程从一个通道（Channel）读取数据并将其存入缓冲区（Buffer），然后可以进行其他任务。当数据准备好时，线程可以继续处理它。NIO使用选择器（Selector）来监听多个通道的I/O事件，从而实现非阻塞I/O。在Java中，NIO的核心类包括Channel、Buffer和Selector。
>
