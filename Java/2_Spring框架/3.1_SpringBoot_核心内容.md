...
[自定义Starter项目]: https://www.bilibili.com/video/BV1xu4y1m7UP?p=20&amp;vd_source=2e87b0aa5bc3613c322738bf9ed03bff
...
因此我们也将我们自己的starter这样设计，先新建一个空的SpringBoot项目，把父工程中的依赖、运行配置、资源等等都删掉：

<img src="./img/1.png" style="zoom: 67%;" />

然后创建并设计三个模块：

- spring-boot-hello：基础业务功能模块（提前把需要的类注册为Bean）
- spring-boot-starter-hello：启动器（这里面不写任何代码）
...
starter启动器主要做依赖管理，这里就不写任何代码，只写pom文件：
...
@ConditionalOnWebApplication    // 测试用，只有在项目使用Web启动器开启的情况下才会生效
...
public class HelloWorldAutoConfiguration {
    ...
    @Bean
  	@ConditionalOnMissingBean    // 测试用，只有在没有Bean存在的情况下才会生效
...
接着在 resource 下编写`META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports`文件，并将我们的自动配置类添加即可：

```properties
com.test.autoconfigure.HelloWorldAutoConfiguration
```

最后再Maven根项目执行`clean`并`install`安装到本地仓库，完成。接着就可以在其他项目中使用我们编写的自定义starter了。

![](./img/3.png)

然后就可以在`application.yml`中测试：
